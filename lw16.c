...
int find_file ( char **adresses, int level, int *adress_num, char **argv, char *dir_name, int a );
...
int find_file ( char **adresses, int level, int *adress_num, char **argv, char *dir_name, int a ) {
	...
	while ( 1 ) {
		...
		if ( dir_str->d_type == DT_REG  ) {
			...
			if ( dir_str->d_type ==  DT_DIR && level > 0 && a == 0 && ( strcmp(pathname,"./.") != 0 ) && ( strcmp(pathname,"./..")  != 0 ) ) {

				find_file ( adresses, level - 1, adress_num, argv, pathname, a+1 );
...
int main ( int argc, char** argv ) {
	...
	int level = (int)argv[2][0], adress_num = 0, a = 0;
	...
	find_file ( adresses, level-1, &adress_num, argv, argv[1], a );
